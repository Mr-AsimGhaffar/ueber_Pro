...
export default function CompanyPage({
  ...
  useEffect(() => {
    const handleFetch = async () => {
      if (tripId) {
        // Fetch offers for a specific trip
        await getTripOffers(Number(tripId));
      } else {
        // Fetch all trips based on current filters
  ...
  const columns: ColumnsType<Offer> = [
    ...
    },
    {
      ...
      className: "font-workSans",
      render: (_, record) => (
        <UpdateStatus offerId={record.id} refreshData={fetchTrips} />
      ),
    },
  ];

  const getTripOffers = async (offerId: number) => {
    try {
      const response = await fetch(
        `/api/offers/getOffersForTrip?id=${offerId}`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
